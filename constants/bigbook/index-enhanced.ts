...
const loadMarkdownContent = (): Record<string, string> => {
  const content: Record<string, string> = {};
  
  try {
    // Load all chapter files
    const chapterFiles = [
      { id: 'foreword-first', file: 'aa-foreword-first.md' },
      { id: 'doctors-opinion', file: 'aa-doctors-opinion.md' },
      { id: 'chapter-1', file: 'aa-chapter-1.md' },
      { id: 'chapter-2', file: 'aa-chapter-2.md' },
      { id: 'chapter-3', file: 'aa-chapter-3.md' },
      { id: 'chapter-4', file: 'aa-chapter-4.md' },
      { id: 'chapter-5', file: 'aa-chapter-5.md' },
      { id: 'chapter-6', file: 'aa-chapter-6.md' },
      { id: 'chapter-7', file: 'aa-chapter-7.md' },
      { id: 'chapter-8', file: 'aa-chapter-8.md' },
      { id: 'chapter-9', file: 'aa-chapter-9.md' },
      { id: 'chapter-10', file: 'aa-chapter-10.md' },
      { id: 'chapter-11', file: 'aa-chapter-11.md' },
      { id: 'appendix-spiritual-experience', file: 'appendix-spiritual-experience.md' }
    ];
    
    for (const chapter of chapterFiles) {
      try {
        const filePath = join(process.cwd(), 'constants', 'bigbook', 'chapters', chapter.file);
        content[chapter.id] = readFileSync(filePath, 'utf-8');
      } catch (error) {
...
// Big Book text content with 1st edition files
export const bigBookTextContent: BigBookTextContent[] = [
  ...
  {
    id: 'appendix-spiritual-experience',
    title: 'Spiritual Experience',
    content: '',
    searchable: true,
    description: 'Appendix II - Understanding different types of spiritual experiences'
  }
];

// Load actual content into the text content objects
const allMarkdownContent = loadMarkdownContent();
...
export const navigateToPageWithHighlight = (
  ...
  try {
    ...
    // Find the specific page marker
    const pageRegex = new RegExp(`--- \\*Page ${pageNumber}\\* ---`, 'i');
    ...
    const pageContent = content.slice(pageStartIndex, pageEndIndex);
