#!/usr/bin/env python3
"""
Big Book Extractor - Using Known Paragraph Positions

Extracts chapters based on manually identified paragraph positions
from the Word document structure.
"""
...
def extract_chapter_content(paragraphs: List[str], start: int, end: int) -> List[str]:
    ...
    for para in paragraphs[start:end]:
        ...
        # Skip all-caps headings (but keep if they're part of actual content)
        if para.isupper() and len(para) < 60 and not any(word in para for word in ['THE', 'HAVE', 'BEEN', 'WITH']):
            continue
...
def main():
    ...
    for chapter_info in CHAPTERS:
        print(f"\n📝 {chapter_info['title']}")
        print(f"   Paragraphs {chapter_info['start']}-{chapter_info['end']}")
